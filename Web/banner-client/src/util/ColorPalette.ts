...
export function findNearestColor(color: string) {
    function RGB(v: string) { return (v.slice(1).match(/\w{2}/g) || []).map(x => parseInt(x, 16)) }
    const rgb = RGB(color);
    function MSE(text: string) { return rgb.map((x, i) => Math.pow(x - RGB(text)[i], 2)).reduce((a, b) => a + b, 0); }
    return Object.entries(ColorPalette).map(([k, v]) => [k, MSE(v)]).sort((a, b) => Number(a[1]) - Number(b[1])).map(x => x[0]).slice(0, 5);
}