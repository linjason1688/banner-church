          ...
          <div class="col-12">
            ...
            <c-column label="手機號碼" v-if="!xform.resetByEmail">
              <c-row>
                ...
                <div class="col-12 col-md-6">
                  <c-input placeholder="請輸入手機號碼" v-model="form.account"
                    :rules="[(val: string) => !!val || '請輸入手機號碼']" dense/>
...
export default class ResetPwd extends ComponentBase {
  ...
  async send() {
    ...
    if (this.xform.resetByEmail == true) {
      await this.apis.feature.user.emailPassword(form.account).catch()
    }
    else {
      await this.apis.feature.user.phonePassword("+"+form.cellAreaCode+form.account).catch()
    }
